...
interface CelebrityDetailModalProps {
  isOpen: boolean
  onClose: () => void
  celebrity: {
    name: string
...
export function CelebrityDetailModal({ isOpen, onClose, celebrity }: CelebrityDetailModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto p-0 sm:rounded-lg">
        ...
        <div className="p-6">
          <DialogHeader>
            <DialogTitle className="font-serif text-2xl font-bold text-gray-800">{celebrity.name}</DialogTitle>
          </DialogHeader>

          <div className="mt-4 space-y-4 text-gray-700">
            <p>{celebrity.fullDescription}</p>
          </div>
        </div>
      </DialogContent>
    </Dialog>
